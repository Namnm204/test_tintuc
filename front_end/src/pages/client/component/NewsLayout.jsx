import { useEffect, useState } from "react";
import { Link } from "react-router-dom";

/* eslint-disable react/prop-types */
const NewsLayout = ({ tintucs }) => {
  return (
    <>
      <div className="w-[90%] md:w-[75%] h-[100%] justify-between flex mx-auto flex-col md:flex-row">
        {/* Phần bên trái */}
        {tintucs.length > 0 ? (
          <div className="w-full md:w-[75%] pr-0 md:pr-4">
            {/* Phần trên */}
            <Link to={`/tintuc/${tintucs[0].id}`} className="w-full">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="col-span-1 md:col-span-2 flex flex-col md:flex-row">
                  <img
                    src={tintucs[0].image} // Hiển thị hình ảnh của tin tức đầu tiên
                    className="w-full md:w-[70%] h-[250px] md:h-[400px] object-cover"
                    alt={tintucs[0].title} // Sử dụng tiêu đề làm alt
                  />
                  <div className="bg-gray-300 w-full md:w-[45%] p-4 overflow-hidden">
                    <h3 className="font-semibold text-lg">
                      ID: {tintucs[0].id}
                    </h3>
                    <h3 className="font-semibold text-lg">
                      {tintucs[0].title}
                    </h3>
                    <p className="whitespace-normal break-words">
                      {tintucs[0].description}
                    </p>
                  </div>
                </div>
              </div>
            </Link>

            {/* Phần dưới */}
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-4">
              {tintucs.slice(1, 4).map((tintuc) => (
                <Link
                  to={`/tintuc/${tintuc.id}`}
                  key={tintuc.id}
                  className="w-full"
                >
                  <div className="bg-gray-300 p-4 overflow-hidden whitespace-normal break-words">
                    <h3>{tintuc.title}</h3>
                    <p className="h-[224px] text-[13px] overflow-hidden text-ellipsis line-clamp-3">
                      {tintuc.description}
                    </p>
                  </div>
                </Link>
              ))}
            </div>
          </div>
        ) : (
          <div>Không có tin tức để hiển thị</div>
        )}

        {/* Phần bên phải */}
        {tintucs.length > 0 ? (
          // <div className="bg-gray-300 relative w-full md:w-[23%] flex flex-col justify-center overflow-y-auto mt-4 md:mt-0">
          <div className="bg-gray-300 relative w-full md:w-[23%] flex flex-col justify-center mt-4 md:mt-0 overflow-y-auto ">
            <h2 className="text-xl font-semibold absolute top-2 bg-blue-500 p-2 pr-14">
              Slide tin tức hot chạy
            </h2>
            {tintucs.map((tintuc, index) => (
              <Link
                to={`/tintuc/${tintuc.id}`}
                key={tintuc.id}
                className="w-full"
              >
                <div key={index} className="flex items-center p-2 ">
                  <img
                    src={tintuc.image}
                    className="w-[100px] h-[80px]"
                    alt=""
                  />
                  <p className="ml-3 ">{tintuc.title}</p>
                </div>
              </Link>
            ))}
          </div>
        ) : (
          <div>Không có tin tức để hiển thị</div>
        )}
      </div>

      <div className="h-px w-[90%] md:w-[75%] mx-auto bg-gray-300 my-8" />
    </>
  );
};

export default NewsLayout;
